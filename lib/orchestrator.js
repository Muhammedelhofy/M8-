...
// Was the previous assistant turn our grounding-decline offer?
function lastWasGroundingOffer(history) {
  const h = Array.isArray(history) ? history : [];
  for (let i = h.length - 1; i >= 0; i--) {
    const m = h[i];
    if (!m || m.role !== "assistant" || typeof m.content !== "string") continue;
    return m.content.includes(GROUNDING_SENTINEL);
  }
  return false;
}
      ...
      try {
        ...
        if (_personal && _label && _label.length >= 2) {
          ...
          try {
            ...
            const _r = await searchKnowledgeGraph(effectiveMessage, 6);
