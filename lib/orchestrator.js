...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (intent === INTENT.NONE && !computeMode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !stateCtx.text && !notebookCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !stateCtx.text && !notebookCtx.text) {
    ...
    // ── MULTI-COMPANY: inject the relevant company context / roster (additive,
    //    like a playbook — no suppression). Keeps M8 from assuming every business
    //    question is the Bolt fleet and from inventing unprofiled companies. ──
    const companyCtx = buildCompanyContext(effectiveMessage);
    if (companyCtx.text) {
      systemInstruction += `\n\n${companyCtx.text}`;
      log("company_context", { company: companyCtx.company, mode: companyCtx.mode });
    }
    ...
    const useCompute = (computeMode || routerCompute || effectiveTutorMode) && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    const computeContract = (computeMode || routerCompute) && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    let notebookCtx = { text: "", mode: null, data: null };
    try { notebookCtx = await buildNotebookContext(effectiveMessage, history, sessionId); } catch (e) { /* non-fatal */ }
    ...
    const conversational = /^(hi|hello|hey|yo|thanks|thank you|thx|ok|okay|cool|nice|great|good (morning|afternoon|evening|night)|salam|سلام|شكرا|مرحبا|تمام|أهلا)\b/i
      .test(effectiveMessage.trim());
    const streamable = !!(fleetCtx.text || financeCtx.text || eosbCtx.text || stateCtx.text || notebookCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
