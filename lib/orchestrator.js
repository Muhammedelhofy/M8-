...
const { detectDiscovery, detectFollowUpLoop, buildDiscoveryDirective, buildDiscoveryNote,
        ...
        detectOEISProbe, buildOEISDirective, buildOEISNotes } = require("./discovery");
...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (!discoveryMode && !explicitLean && !m1Mode) {
      try {
    ...
    if (!discoveryMode && !oeisMode && !m1Mode) {
      try {
    ...
    const _tNb = Date.now();
    if (!discoveryMode && !oeisMode && !leanMode && !m1Mode) {
      try {
    ...
    if (!discoveryMode && !oeisMode && !leanMode && !m1Mode && !notebookCtx.text && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text) {
      try {
    ...
    if (intent === INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !m1Mode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !m1Mode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text) {
    ...
    if (compoundMode && !discoveryMode && !oeisMode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text) {
      trace.searchExecuted = true;
      try {
        const _tSearch = Date.now();
    ...
    // !m1Mode: the M1 census is already computed deterministically in-process —
    // letting Gemini re-run its own code against the packet invites divergence.
    const useCompute = (computeMode || routerCompute || effectiveTutorMode || discoveryMode || oeisMode || compoundMode) && !m1Mode && !fleetCtx.text && !notebookCtx.text && !graphCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    const computeContract = (computeMode || routerCompute || discoveryMode || oeisMode) && !compoundMode && !m1Mode && !fleetCtx.text && !notebookCtx.text && !graphCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    if (openProblem && !discoveryMode && !oeisMode && !m1Mode) systemInstruction += `\n\n${OPEN_PROBLEM_DIRECTIVE}`;
    ...
    if (m1Mode && response !== FALLBACK_RESPONSE) {
      try {
        for (const note of m1Run.notes) await persistNote(sessionId, note);
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    let notebookCtx = { text: "", mode: null, data: null };
    if (!discoveryMode && !leanMode && !m1Mode) { try { notebookCtx = await buildNotebookContext(effectiveMessage, history, sessionId); } catch (e) { /* non-fatal */ } }
    ...
    let graphCtx = { text: "", mode: null, data: null };
    if (!discoveryMode && !leanMode && !m1Mode && !notebookCtx.text && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text) {
      try {
        const { buildGraphContext } = require("./memory-graph");
        graphCtx = await buildGraphContext(effectiveMessage, sessionId);
      } catch (e) { /* non-fatal */ }
    ...
    const streamable = !discoveryMode && !leanMode && !m1Mode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
