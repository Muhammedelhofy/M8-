...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (!discoveryMode && !explicitLean && !m1Mode && !m3Mode) {
      try {
    ...
    if (!discoveryMode && !oeisMode && !m1Mode && !m3Mode) {
      try {
    ...
    const _tNb = Date.now();
    if (!discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode) {
      try {
    ...
    if (!discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode && !notebookCtx.text && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text) {
      try {
    ...
    if (intent === INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !m1Mode && !m3Mode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text) {
    ...
    const useCompute = (computeMode || routerCompute || effectiveTutorMode || discoveryMode || oeisMode || compoundMode) && !m1Mode && !m3Mode && !fleetCtx.text && !notebookCtx.text && !graphCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    const computeContract = (computeMode || routerCompute || discoveryMode || oeisMode) && !compoundMode && !m1Mode && !m3Mode && !fleetCtx.text && !notebookCtx.text && !graphCtx.text && !financeCtx.text && !eosbCtx.text;
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    const streamable = !discoveryMode && !leanMode && !m1Mode && !m3Mode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
    ...
    // Novelty-capability guard (mirrors the buffered path): don't under-claim the LIVE M2 novelty check
    try { if (detectResearchNovelty(message, history)) systemInstruction += `\n\n${NOVELTY_CAPABILITY_DIRECTIVE}`; } catch { /* non-fatal */ }
    if (buildQuery)       systemInstruction += `\n\n${renderBuildState()}`;
