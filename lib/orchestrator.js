...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    // ── TEMP VISIONDBG (remove after vision fix) ──
    try {
      const lastParts = (contents[contents.length - 1]?.parts || []).map((p) =>
        p?.inlineData ? `inlineData(${p.inlineData.mimeType})` : `text(${(p?.text || "").length})`);
      console.error("[VISIONDBG] orchestrate", JSON.stringify({
        imgTurn,
        attCount: Array.isArray(attachments) ? attachments.length : 0,
        attKinds: Array.isArray(attachments) ? attachments.map((a) => a?.kind || (a?.content != null ? "text" : "?")) : [],
        providerOrder: imgTurn ? visionProviderOrder() : "(default)",
        lastParts,
      }));
    } catch (_) {}
    try {
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    const streamable = !hasImageAttachments(attachments) && !discoveryMode && !leanMode && !m1Mode && !m3Mode && !lemmaDagMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
