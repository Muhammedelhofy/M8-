  ...
  if (_wantsBreakdown) {
    try {
      ...
      if (walletSignal && !fleetSignal) {
        ...
        const bd = await _wallet.getCategoryBreakdown(p.start, p.end, who && who.id);
...
function capabilityFallback(message, arb) {
  const m = String(message || "");
...
async function resolveDomainRoute(baseMessage, history) {
  try {
    ...
    return { arb, routedMessage: baseMessage, clarified: false, lookup };
  } catch (_) {
...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    if (intent !== INTENT.DOC) {
      const _cap = capabilityFallback(baseMessage, _arb);
      if (_cap) {
        log("capability_fallback");
        // Build-150: log miss for router improvement (fire-and-forget, never blocks reply)
        const _missLane = _cap.money ? "money" : (_CAP_TASK_RE.test(baseMessage) ? "task" : "note");
        logMiss(baseMessage, _missLane, "phase0_safety_net");
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    if (intent !== INTENT.DOC) {
      const _capS = capabilityFallback(baseMessage, _arbS);
