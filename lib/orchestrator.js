...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    try {
      ...
      if (changeCtx.text) {
        fleetCtx.text = fleetCtx.text ? `${changeCtx.text}\n\n${fleetCtx.text}` : changeCtx.text;
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    try {
      ...
      if (changeCtxS.text) fleetCtx.text = fleetCtx.text ? `${changeCtxS.text}\n\n${fleetCtx.text}` : changeCtxS.text;
    } catch (e) { /* non-fatal */ }
