...
function parseCurrencyConvert(raw) {
  ...
  if (target) {
    const cue = /\b(?:all|everything|put|show|give|make|express|breakdown|spend\w*|spent|total|expenses?|combine)\b/i.test(m)
...
function renderConvertedBreakdown(bd, ar, who, label, target) {
  ...
  const whoPfx = who ? `${who} — ` : "";
  const head = ar ? `${whoPfx}المصاريف${onLbl} (الكل بالـ${target}):` : `${whoPfx}Spending${onLbl} (all in ${target}):`;
...
function lastWalletQueryContext(history) {
  ...
  for (let i = history.length - 1; i >= 0 && i >= history.length - 6; i--) {
    ...
    // member: "Sara's expenses/last/spent/income" or "Sara spent ..." (exclude You/Your)
    const nm = c.match(/\b([A-Z][a-z]+)(?:'s|’s)?\s+(?:expenses?|last|spent|spend|spending|income|net)/);
    const memberName = (nm && !/^(You|Your|Net|Total|Top|Income)$/.test(nm[1])) ? nm[1] : null;
...
async function resolveDomainRoute(baseMessage, history) {
  try {
    ...
    const arb = await _arbiter.arbitrate(baseMessage, {
