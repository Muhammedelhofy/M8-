...
async function buildMorningBriefSlot({ effectiveMessage, history, fleetLike, fleetCtx }) {
  try {
    ...
    let brief = await getTodayBrief();
    if (!brief) brief = await computeLiveBrief();
    ...
    if (askedForBrief) {
      ...
      const directive = behindAsk
        ? `MORNING FLEET BRIEF — the user asked specifically WHO IS BEHIND. ` +
        ...
        : `MORNING FLEET BRIEF — the user asked for the daily brief / how the drivers are doing. ` +
          `Present ALL THREE sections clearly, in this priority order: (1) DROPPED YESTERDAY first if any (the most urgent group), ` +
          `(2) ON TRACK, (3) BELOW TARGET. Use ONLY the ground-truth figures below; never invent a driver or alter a number. ` +
          `Projections are ESTIMATES — say so.\n\n${body}`;
      fleetCtx.text = fleetCtx.text ? `${directive}\n\n${fleetCtx.text}` : directive;
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    try { const { detectBriefEmailCommand } = require("./notify"); briefEmailMode = !!detectBriefEmailCommand(message); } catch (e) { /* non-fatal */ }
    ...
    const streamable = !hasImageAttachments(attachments) && !discoveryMode && !leanMode && !m1Mode && !m3Mode && !lemmaDagMode && !kernelTestMode && !decompProposalMode && !lychrelMode && !engineCatalogMode && !commandCenterMode && !briefEmailMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
