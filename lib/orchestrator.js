...
async function resolveDomainRoute(baseMessage, history) {
  try {
    ...
    // Build-155: ALL-DOMAIN registry router — DORMANT shadow. When M8_REGISTRY_ROUTER=1
    // we ALSO compute the generalised registry decision and log it (lane=arbiter:reg:*)
    // for review, but RETURN the same wallet⇄fleet `arb` above ⇒ ZERO routing change.
    // Deterministic-only (useLLM:false) so it stays free during live testing. The
    // per-boundary flips that ACT on this are Builds 156–158. Default OFF ⇒ this block
    // never runs ⇒ byte-for-byte pre-155 behaviour. logRoute is fire-and-forget + safe.
    ...
    return { arb, routedMessage: baseMessage, clarified: false };
  } catch (_) {
