...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    const streamable = !hasImageAttachments(attachments) && !discoveryMode && !leanMode && !m1Mode && !m3Mode && !lemmaDagMode && !kernelTestMode && !decompProposalMode && !lychrelMode && !engineCatalogMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
