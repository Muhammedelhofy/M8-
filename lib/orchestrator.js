...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (intent === INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text) {
    ...
    if (compoundMode && !discoveryMode && !oeisMode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text) {
      trace.searchExecuted = true;
      try {
        const _tSearch = Date.now();
    ...
    const useCompute = (computeMode || routerCompute || effectiveTutorMode || discoveryMode || oeisMode || compoundMode) && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    const computeContract = (computeMode || routerCompute || discoveryMode || oeisMode) && !compoundMode && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    const streamable = !discoveryMode && !leanMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
