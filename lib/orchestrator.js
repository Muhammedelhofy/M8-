...
const COMPUTE_HEURISTIC = /\b(factorial|fibonacci|how many (?:primes?|digits|combinations|permutations)|\d+\s*!\B|prime factor|nth (?:prime|digit)|verify\s+\w+\s+(?:up\s+)?to\s+\d|sum of (?:the\s+)?(?:first|all|integers)|monte[\s-]?carlo|standard deviation|compound (?:interest|growth)|amortiz)/i;
...
// ── PERIOD-COMPARISON WINDOW INTEGRITY (silent-fail guard on fleet turns) ─────
// On a fleet turn that compares two time periods, the LLM tends to compare raw
// TOTALS even when the windows differ in length or one is still partial (a few
// days of this month vs a full prior month) — the misleading "we already beat
// last month in 7 days" lead. The general LIKE-FOR-LIKE rule in the system prompt
// works on plain turns but gets drowned by the big fleet packet at low temp, so
// we re-assert it ADJACENT to the data when a period comparison is detected. The
// detection is deterministic; the fleet numbers themselves are never touched.
const CMP_VERB   = /\b(compare|comparison|vs\.?|versus|against|better|worse|beat(?:en)?|outperform\w*|ahead|behind|on\s+track|keep(?:ing)?\s+up|catch(?:ing)?\s+up|stack\s+up)\b/i;
const CMP_PERIOD = /\b(this|last|previous|current)\s+(week|month|quarter|year)\b|\bmonth[- ]?to[- ]?date\b|\bmtd\b|\bso\s+far\b/i;
// Inherently comparative period phrasing (month-over-month, week-on-week, …):
// triggers on its own — no separate comparison verb needed.
const CMP_SELF = /\b(?:week|month|year)[\s-]?(?:over|on|to|vs\.?)[\s-]?(?:week|month|year)\b/i;
function isPeriodComparison(message) {
  const m = message || "";
  if (CMP_SELF.test(m)) return true;
  return CMP_VERB.test(m) && CMP_PERIOD.test(m);
}
const WINDOW_INTEGRITY_DIRECTIVE = `WINDOW INTEGRITY (this turn compares time periods): Check the two periods are the SAME length and both COMPLETE before comparing. If they are NOT — a partial / in-progress window (a few days of the current week or month) against a full prior one, or windows of different length — say so FIRST, then compare on DAILY RATE or pro-rated PACE, never raw totals. Do NOT headline a partial-vs-full total as a win or loss ("we already beat last month in just 7 days", "we're behind last week"). Lead with the rate-based read.`;
...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    // Period comparison on a fleet turn → re-assert like-for-like adjacent to the
    // data (independent of override/false-consensus; a comparison is neither).
    if (fleetCtx.text && isPeriodComparison(effectiveMessage)) {
      fleetCtx.text = `${fleetCtx.text}\n\n${WINDOW_INTEGRITY_DIRECTIVE}`;
      log("window_integrity");
    }
    ...
    if (intent === INTENT.NONE && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !stateCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    //    the fleet packet is already authoritative; don't recompute it). ──
    const useCompute = computeMode && !fleetCtx.text;
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    const conversational = /^(hi|hello|hey|yo|thanks|thank you|thx|ok|okay|cool|nice|great|good (morning|afternoon|evening|night)|salam|سلام|شكرا|مرحبا|تمام|أهلا)\b/i
      .test(effectiveMessage.trim());
    const streamable = !!(fleetCtx.text || stateCtx.text || openProblem || buildQuery || conversational || isPersonal(effectiveMessage));
