...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (intent === INTENT.DOC && !fleetLike && !notebookLike) {
      log("docgen_start");
      try {
    ...
    if (intent === INTENT.NONE && !computeMode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !stateCtx.text && !notebookCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !fleetCtx.text && !fleetLike && !stateCtx.text && !notebookCtx.text) {
    ...
    const useCompute = (computeMode || routerCompute || effectiveTutorMode) && !fleetCtx.text && !notebookCtx.text;
    ...
    // layer's compute pick (NOT tutor — keeps Socratic flow; NOT fleet/notebook — own packets).
    const computeContract = (computeMode || routerCompute) && !fleetCtx.text && !notebookCtx.text;
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    const conversational = /^(hi|hello|hey|yo|thanks|thank you|thx|ok|okay|cool|nice|great|good (morning|afternoon|evening|night)|salam|سلام|شكرا|مرحبا|تمام|أهلا)\b/i
      .test(effectiveMessage.trim());
    const streamable = !!(fleetCtx.text || stateCtx.text || notebookCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
    ...
    const useCompute = (computeMode || effectiveTutorMode) && !fleetCtx.text && !notebookCtx.text;
    if (useCompute)       systemInstruction += `\n\n${COMPUTE_DIRECTIVE}`;
    if (computeMode && !fleetCtx.text && !notebookCtx.text) systemInstruction += `\n\n${verifiedOutputContract("compute")}`;
    if (openProblem)      systemInstruction += `\n\n${OPEN_PROBLEM_DIRECTIVE}`;
    if (buildQuery)       systemInstruction += `\n\n${renderBuildState()}`;
