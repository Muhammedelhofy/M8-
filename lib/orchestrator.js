...
function stripMoneyHistory(history) {
  ...
  return history.filter((h) => {
    ...
    if (h.role === "user" && (parseAddExpense(c) || parseSpendQuery(c) || parseEditExpense(c))) return false;
    return true;
  });
}
...
async function handleWalletCommand(message, history) {
  ...
  // The keyword parsers above all missed. If this still looks money-ish, ask a fast
  // free model to UNDERSTAND it (privacy: live message ONLY, no stored data/history,
  // not logged), then map its proposal to the SAME confirm-gated actions. The model
  // gets NO new authority. Low confidence / unavailable → fall through to null → the
  // Phase 0 capability net answers. Kill switch: M8_INTENT_BRAIN_DISABLED=1.
  if (_MONEY_PLAUSIBLE.test(m) && !looksFleet(m) && !looksFinance(m)) {
    const it = await classifyMoneyIntent(m, _wallet.EXPENSE_CATEGORIES);
    if (it && it.confidence >= 0.6) {
      if (it.kind === "add" && it.amount > 0) {
        const cur = it.currency || "SAR";
        const cat = it.category || _wallet.inferCategory(m);
        const noteStr = it.note ? " · " + it.note : "";
        return ar
          ? `🧾 تأكيد مصروف — أضيف ${fmtAmt(it.amount, cur)} · ${cat}${noteStr}؟ اكتب «نعم» للتأكيد أو «لا» للإلغاء.`
          : `🧾 Confirm expense — add ${fmtAmt(it.amount, cur)} · ${cat}${noteStr}? Reply "yes" to log it (or "no" to cancel).`;
      }
      if (it.kind === "edit_last" && (it.amount > 0 || it.category)) {
        try {
          const last = await _wallet.getLastM8Write();
          if (!last || !last.wallet_txn_id) return ar ? "ما في مصروف أضفته من M8 مؤخرًا لأعدّله." : "I don't have a recent M8-added expense to edit.";
          const oldStr = `${fmtAmt(last.amount, last.currency)} · ${last.category || "Other"}`;
          const parts = [];
          if (it.amount > 0) parts.push(fmtAmt(it.amount, last.currency));
...
async function handleNotesCommand(message, history) {
  ...
  if (cap) {
    try { await _notes.addNote(cap); return ar ? "📝 سجّلتها." : "📝 Noted."; }
...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    const _tk = await handleTasksCommand(baseMessage);
    if (_tk !== null) { log("tasks_cmd"); return _tk; }
    ...
    const entityCardSuppressSearch = !!entityCard;
    if (entityCardSuppressSearch) log("entity_card_search_suppressed", { entity: entityCardName });
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    const _tkS = await handleTasksCommand(baseMessage);
    if (_tkS !== null) { emit(_tkS); return _tkS; }
