...
function renderRecentExpenses(list, ar) {
  if (!list || !list.length) {
  ...
  if (list.length === 1) {
    ...
    return ar ? `آخر مصروف: ${fmtAmt(e.amount, e.currency)} · ${e.category}${tail}.`
              : `Your last expense: ${fmtAmt(e.amount, e.currency)} · ${e.category}${tail}.`;
  }
  const lines = list.map((e) => {
    const when = fmtRelDate(e.occurredOn, ar);
    return `• ${fmtAmt(e.amount, e.currency)} · ${e.category}${when ? " · " + when : ""}`;
  });
  return (ar ? `آخر ${list.length} مصاريف:\n` : `Your last ${list.length} expenses:\n`) + lines.join("\n");
}
...
async function handleWalletCommand(message, history) {
  ...
  if (recent) {
    try {
      const list = await _wallet.getRecentExpenses(recent.n);
      return renderRecentExpenses(list, ar);
    } catch (e) {
  ...
  if (m.length <= 200 && (_MONEY_PLAUSIBLE.test(m) || refish) && !looksFleet(m) && !looksFinance(m)) {
    ...
    if (it && it.confidence >= 0.5 && it.kind !== "unknown") {
      ...
      if (it.kind === "last_expense") {
        try {
          const list = await _wallet.getRecentExpenses(1);
          return renderRecentExpenses(list, ar);
        } catch (e) { return ar ? "ما قدرت أقرأ المحفظة الآن." : "I couldn't read your wallet right now."; }
      }
