...
async function handleTasksCommand(message) {
  ...
  try {
    ...
    if (cmd.op === "add") {
      ...
      return ar ? `تمام، أضفت: «${data.title}»${due}.` : `Added to your list: "${data.title}"${due}.`;
    }

    if (cmd.op === "list") {
      const { data, error } = await db.from("m8_tasks").select("*").eq("done", false)
        .order("due_at", { ascending: true, nullsFirst: false }).order("created_at", { ascending: false }).limit(20);
      if (error) throw error;
      const open = data || [];
      if (!open.length) return ar ? "قائمتك فاضية 🎉" : "Your list is empty 🎉";
      const lines = open.map((t, i) => {
        const due = t.due_at ? (ar ? " — " + _taskDueAr(t.due_at) : " — " + _taskDueEn(t.due_at)) : "";
        return `${i + 1}. ${t.title}${due}`;
      });
      const head = ar ? `عندك ${open.length} مهمة مفتوحة:` : `You have ${open.length} open task${open.length === 1 ? "" : "s"}:`;
      return head + "\n" + lines.join("\n");
    }
    ...
    if (cmd.op === "done") {
      ...
      if (error) throw error;
      return ar ? `تمام، علّمت «${target.title}» كمكتملة ✓` : `Marked "${target.title}" as done ✓`;
    }
    if (cmd.op === "delete") {
...
// code-computed totals (lib/wallet.js). PRIVACY WALL: replies carry
// MONEY_SENTINEL so stripMoneyHistory() removes them — and the user's money
// commands — from the history sent to the LLM. Wallet amounts/text must NEVER
// reach a model prompt or a log.
...
function stripMoneyHistory(history) {
  ...
  return history.filter((h) => {
    ...
    if (h.role === "user" && (parseAddExpense(c) || parseSpendQuery(c))) return false;
    return true;
  });
}
