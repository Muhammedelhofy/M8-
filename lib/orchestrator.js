...
const { detectLeanProbe, buildLeanNotes, runLeanTurn } = require("./lean");
...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    const _tNb = Date.now();
    if (!discoveryMode && !oeisMode && !leanMode) {
      try {
    ...
    if (!discoveryMode && !oeisMode && !leanMode && !notebookCtx.text && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text) {
      try {
    ...
    if (intent === INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text) {
    ...
    const useCompute = (computeMode || routerCompute || effectiveTutorMode || discoveryMode || oeisMode || compoundMode) && !fleetCtx.text && !notebookCtx.text && !graphCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    const computeContract = (computeMode || routerCompute || discoveryMode || oeisMode) && !compoundMode && !fleetCtx.text && !notebookCtx.text && !graphCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    if (openProblem && !discoveryMode && !oeisMode) systemInstruction += `\n\n${OPEN_PROBLEM_DIRECTIVE}`;
    ...
    if (discoveryMode && response !== FALLBACK_RESPONSE) {
      try {
        ...
        // Both paths: suggest next probe + log next_step singleton
        const suggestion = suggestNextProbe({ lastBound, foundCounter, thread: discovery.thread });
        if (suggestion) {
          try {
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    let notebookCtx = { text: "", mode: null, data: null };
    if (!discoveryMode && !leanMode) { try { notebookCtx = await buildNotebookContext(effectiveMessage, history, sessionId); } catch (e) { /* non-fatal */ } }
    ...
    let graphCtx = { text: "", mode: null, data: null };
    if (!discoveryMode && !leanMode && !notebookCtx.text && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text) {
      try {
        const { buildGraphContext } = require("./memory-graph");
        graphCtx = await buildGraphContext(effectiveMessage, sessionId);
      } catch (e) { /* non-fatal */ }
    ...
    const streamable = !discoveryMode && !leanMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
