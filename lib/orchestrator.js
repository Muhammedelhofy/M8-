...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    // ── Build-168 (E2 step 1): context-packet telemetry — MEASURE only, never
    //    gates or reorders anything. Sizes+labels only (no content, no PII).
    //    Kill switch: M8_CTX_TELEMETRY=off. Insert awaited (bounded ≤1.5s).
    try {
      ...
      await recordPacket({ systemInstruction, history, lane: ctxLane });
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    try {
      ...
      await recordPacket({ systemInstruction, history, lane: "stream:" + ctxLaneS });
    } catch (_) { /* telemetry never blocks the reply */ }
