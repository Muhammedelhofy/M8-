...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (!discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !lemmaDagMode) {
      try {
    ...
    if (intent === INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !loopCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !m1Mode && !m3Mode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !loopCtx.text) {
    ...
    if (pastMemory.length > 0) {
      ...
      // Build-26: when the loop-recall lane is active, strip contaminated memory
      // entries that assert per-ID triage verdicts (e.g. "Conjecture #7 was kept").
      // Prior battery runs stored confabulated responses into m8_conversations; the
      // model was trusting those over the grounding packet injected later. This
      // filter is scoped tightly: only fires when loopCtx.text is non-empty.
      // Round4 fix: scope to model+summary rows only — user turns are never filtered
      const LOOP_TRIAGE_CONTAMINATION = /#\s*\d+[^\n]{0,150}(?:kept|dismissed)/i;
      const filteredMemory = loopCtx.text
        ? pastMemory.filter(m => {
      ...
      if (filteredMemory.length > 0) {
        const memoryBlock = filteredMemory
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    const streamable = !discoveryMode && !leanMode && !m1Mode && !m3Mode && !lemmaDagMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || graphCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
