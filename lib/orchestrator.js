...
const { classifyIntent, selectSources, mergeEvidence, renderEvidenceBlock, toItems } = require("./answer-engine");
...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    // ── Build-87: Driver Cost Profiles — real net P&L overlay ────────────────
    // Append monthly cost-per-driver to the finance packet so M8 can compute
    // REAL net P&L (gross - rental - salary - fuel - other) rather than gross-only.
    // Only injected when the finance lane already owns the turn (financeCtx.text set).
    if (financeCtx.text) {
      try {
        ...
        if (profiles && profiles.length > 0) {
          ...
          const profileLines = profiles.map((p) => {
            const total = (Number(p.rental_amount||0) + Number(p.salary_amount||0) + Number(p.fuel_estimate||0) + Number(p.other_costs||0));
            return `  ${p.driver_name}: rental ${fmtNum(p.rental_amount)} + salary ${fmtNum(p.salary_amount)} + fuel ${fmtNum(p.fuel_estimate)} + other ${fmtNum(p.other_costs)} = ${total.toFixed(0)} SAR/mo${p.notes ? ` (${p.notes})` : ""}`;
          }).join("\n");
          financeCtx.text +=
            `\n\nDRIVER COST PROFILES (Build-87 — monthly costs in SAR):\n` +
            `${profileLines}\n` +
            `Real net P&L = gross_earnings − monthly_costs (divide by 4.33 for weekly). ` +
            `These are GROUND TRUTH — never invent costs for a driver. ` +
            `If a driver has NO profile here, their cost structure is unknown: do NOT invent it.`;
    ...
    if (intent === INTENT.NONE && !imgTurn && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode && !isPersonal(effectiveMessage) && !conversational && !hasDocAttachment && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !loopCtx.text && !openProblem && !buildQuery) {
      try {
        ...
        const decision = await decideAction({ message: effectiveMessage, history, topicHint });
    ...
    if (kgGateOpen) {
      try {
        const cls = await classifyIntent(effectiveMessage);
