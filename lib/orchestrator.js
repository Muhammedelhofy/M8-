...
const VERIFIED_OUTPUT_CONTRACT = `VERIFIED-OUTPUT CONTRACT (a truth-tool computed this answer — carry the proof, don't pad it):
1) RESULT — lead with the computed answer plainly.
2) VERIFICATION — in a few words, name that it was executed ("computed in Python", "ran the code"), so it's clearly a run result, not an estimate.
3) CONFIDENCE — say "high" for a clean deterministic computation; flag it lower and say WHY if the code assumed an input, sampled (Monte Carlo), or rounded.
4) METHOD/SOURCE — one short phrase on the method or the inputs used.
LOAD-BEARING RULE — narration must NOT exceed the evidence: report exactly what the code printed; never add, extrapolate, round away, or "interpret" beyond the printed output. If the code errored or didn't actually produce the figure, SAY SO — never paper over it with a plausible number. Keep it tight and voice-first: lead with the result, then a compact verification tag — do NOT read the four fields aloud like a checklist.`;
...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    // L4 contract: only the real compute lane (NOT tutor — keeps Socratic flow; NOT fleet — has its own packet).
    const computeContract = computeMode && !fleetCtx.text;
    if (computeContract) { systemInstruction += `\n\n${VERIFIED_OUTPUT_CONTRACT}`; log("l4_contract"); }
