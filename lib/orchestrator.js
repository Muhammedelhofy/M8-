...
function capabilityFallback(message, arb) {
  ...
  const ar = isArabic(m);
  // Build-152: trust the arbiter's wallet/fleet call; else the greedy looksFleet.
  const _fleetVeto = (arb && arb.domain === "wallet") ? false
                   : (arb && arb.domain === "fleet")  ? true
                   : looksFleet(m);
...
async function resolveDomainRoute(baseMessage, history) {
  try {
    ...
    const arb = await _arbiter.arbitrate(baseMessage, _hints);

    // Build-155/156: ALL-DOMAIN registry classifier. Computed ONCE when EITHER the
    // dormant shadow (M8_REGISTRY_ROUTER) OR the lookup flip (M8_REGISTRY_LOOKUP) is on.
    ...
    if (process.env.M8_REGISTRY_ROUTER === "1" || process.env.M8_REGISTRY_LOOKUP === "1") {
      ...
      } catch (_) { /* shadow/lookup must never affect the turn */ }
    }
