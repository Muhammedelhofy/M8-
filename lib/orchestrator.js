...
const { buildNotebookContext, persistNote, looksNotebook } = require("./notebook");
const { detectDiscovery, buildDiscoveryDirective, buildDiscoveryNote } = require("./discovery");
...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (intent === INTENT.DOC && !fleetLike && !notebookLike && !financeLike) {
      log("docgen_start");
      try {
    ...
    try {
      ...
      if (discovery.discovery) log("discovery_run", { thread: discovery.thread, bound: discovery.bound });
    } catch (dErr) {
    ...
    // SKIPPED on a discovery turn (discovery owns the fused compute+log flow).
    let notebookCtx = { text: "", mode: null, data: null };
    const _tNb = Date.now();
    if (!discoveryMode) {
      try {
        notebookCtx = await buildNotebookContext(effectiveMessage, history, sessionId);
        if (notebookCtx.text) log("notebook_context", { notebookMode: notebookCtx.mode });
      } catch (nbErr) {
    ...
    if (intent === INTENT.NONE && !computeMode && !discoveryMode && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !openProblem && !buildQuery) {
      try {
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !computeMode && !discoveryMode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text) {
    ...
    const useCompute = (computeMode || routerCompute || effectiveTutorMode || discoveryMode) && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    const computeContract = (computeMode || routerCompute || discoveryMode) && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
    ...
    if (discoveryMode) { systemInstruction += `\n\n${buildDiscoveryDirective(discovery.thread, discovery.bound)}`; log("discovery_directive"); }

    // ── OPEN-PROBLEM HONESTY: force the honest "can't" lead (this turn) ──
    if (openProblem && !discoveryMode) systemInstruction += `\n\n${OPEN_PROBLEM_DIRECTIVE}`;
    ...
    // ── PHASE 4 DISCOVERY: stage the ledger entry FROM THE COMPUTED RESPONSE and
    //    persist it once. A failed run (fallback / no execution marker) stages
    //    nothing — an unverified claim never enters the ledger as evidence. A
    //    found counterexample is recorded as kind 'counterexample' (importance 5).
    if (discoveryMode && response !== FALLBACK_RESPONSE) {
      try {
        ...
        if (dnote) {
          ...
          log("discovery_not_logged");   // no exec marker — honest: nothing verified, nothing logged
        }
...
async function orchestrateStream({ message, sessionId, history, onChunk, onReset }) {
  ...
  try {
    ...
    try { discoveryMode = !!detectDiscovery(message).discovery; } catch (e) { /* non-fatal */ }

    // Research notebook (hard-route like fleet/state — code owns the ledger).
    let notebookCtx = { text: "", mode: null, data: null };
    if (!discoveryMode) { try { notebookCtx = await buildNotebookContext(effectiveMessage, history, sessionId); } catch (e) { /* non-fatal */ } }
    ...
    const streamable = !discoveryMode && !!(fleetCtx.text || financeCtx.text || eosbCtx.text || companyCtx.text || stateCtx.text || notebookCtx.text || openProblem || buildQuery || effectiveTutorMode || conversational || isPersonal(effectiveMessage));
    ...
    const useCompute = (computeMode || effectiveTutorMode) && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text;
    if (useCompute)       systemInstruction += `\n\n${COMPUTE_DIRECTIVE}`;
    if (computeMode && !fleetCtx.text && !notebookCtx.text && !financeCtx.text && !eosbCtx.text) systemInstruction += `\n\n${verifiedOutputContract("compute")}`;
