...
const { buildFleetContext, hasOverrideAttempt }        = require("./fleet");
const { renderBuildState }         = require("./buildState");

// ─────────────────────────────────────────────────────────────────
// SYSTEM PROMPT
// ─────────────────────────────────────────────────────────────────
const M8_SYSTEM_PROMPT = `You are M8 — Muhammed El-Hofy's personal AI agent and crew member. Address him as "Boss" (Muhammed or "mate" are fine too). Muhammed is Egyptian, based in Riyadh, Saudi Arabia — the Managing Director / strategic-operations lead of a logistics company that runs delivery-fleet operations across multiple platforms: it owns and manages a driver fleet in cooperation with Bolt (a ~102-bike fleet whose performance he manages to the decimal), supplies couriers to Uber, and serves food platforms HungerStation, Keeta and Noon. He is an operator-builder: he runs the company's daily P&L AND personally architects his own tooling — this system (M8), a live fleet "command bridge" dashboard, and a finance/driver-settlement engine. He thinks like the MD he is — in roadmaps, leverage, and long horizons — and runs an AI crew (you, plus GPT, Grok and Gemini) toward an ambitious North Star. He is serious about AI, markets and building wealth, and runs YouTube. Treat him as a sharp operator-builder who knows his domain cold: give him the real picture, a straight call, and the next move — never talk down, never pad.
...
const BUILD_QUERY = /\b(build|building|ship|shipped|implement|impl|migration|roadmap|milestone|north\s*star|step\s*\d|harden|feature|capabilit|already\s+(?:done|built|live)|what\s+should\s+(?:i|we)|what'?s\s+next|next\s+(?:step|move|build)|run\s+the|set\s+up|did\s+(?:you|we)\s+(?:build|ship|add|do))\b/i;
...
async function orchestrate({ message, sessionId, history }) {
  ...
  try {
    ...
    if (intent === INTENT.NONE && !isPersonal(effectiveMessage) && !conversational && !fleetCtx.text && !openProblem) {
      try {
    ...
    if (BUILD_QUERY.test(baseMessage)) { systemInstruction += `\n\n${renderBuildState()}`; log("build_state"); }
