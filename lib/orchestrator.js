...
function appendExportMarker(response, message, hasFleetData) {
  if (response === FALLBACK_RESPONSE || !hasFleetData) return response;
  const fmt = exportIntent(message);
  if (!fmt) return response;
  const label = fmt === "pptx" ? "Download Fleet Deck (PowerPoint)" : "Download Fleet Report (Excel)";
  const spec  = {
...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    if (intent === INTENT.DOC && !imgTurn && !fleetLike && !notebookLike && !financeLike && !hasDocAttachment) {
      log("docgen_start");
      try {
    ...
    finalResponse = appendExportMarker(finalResponse, message, !!fleetCtx?.text);
    try {
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    const streamFinal = appendExportMarker(appendChartMarker(response, fleetCtx), message, !!fleetCtx?.text);
    return streamFinal;
