...
const { buildFleetContext, hasOverrideAttempt, looksFleet } = require("./fleet");
...
async function orchestrateStream({ message, sessionId, history, onChunk }) {
  const t0 = Date.now();
  const meta = {};
  ...
  try {
    ...
    try {
      response = await generateStream({
        systemInstruction,
        contents,
        providerOrder: dr.deep ? DEEP_ORDER : ROUTING[intent],
        genConfig: dr.deep
          ? { temperature: 0.3, maxOutputTokens: DEEP_MAX_TOKENS, geminiModel: DEEP_MODEL, thinkingBudget: DEEP_THINKING_BUDGET }
          : { temperature: fleetCtx.text ? 0.15 : 0.4, maxOutputTokens: 2048 },
        meta,
