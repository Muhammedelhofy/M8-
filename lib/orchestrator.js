...
function capabilityFallback(message, arb, crud) {
  const m = String(message || "");
  if (!m.trim()) return null;
  const ar = isArabic(m);
  // Build-159: a CLEAR registry winner in {tasks, notes, driver_profile} carried from
  // resolveDomainRoute (behind M8_REGISTRY_CRUD, default OFF). It only RESCUES a phrasing
  // that every deterministic keyword lane above already missed — so it BROADENS the noun
  // match below (the ACTION requirement stays, so a no-action chat turn is never stolen).
  const _crud = (crud && crud.domain) || null;
  // Build-152/157: trust the front-door wallet/fleet/finance call; else the greedy
  // looksFleet backstop. A confident fleet OR finance decision vetoes the money card
  // (mirrors handleWalletCommand's central gate so both abstain identically).
  const _fleetVeto = (arb && arb.domain === "wallet") ? false
                   : (arb && (arb.domain === "fleet" || arb.domain === "finance")) ? true
                   : looksFleet(m);
  // MONEY — strong wallet nouns ⇒ wallet intent. Never steal a fleet/finance
  // question (those have their own lanes below this point).
  if (_CAP_MONEY_RE.test(m) && !_fleetVeto && !looksFinance(m)) {
  ...
  // DRIVER PROFILE (Build-159) — ONLY via a CLEAR registry winner. Its strong signal
  // ("driver profile(s)" / "set|update X's rental|salary|fuel") already implies the
  // action, so no separate _CAP_ACTION_RE gate is needed and a bare "driver" never fires.
  if (_crud === "driver_profile") {
    return {
      money: false,
      lane: "driver_profile",
      reply: ar
  ...
  // TASKS — noun + an action signal. Build-159: the registry winner BROADENS the noun
  // match (rescues "remind me …"/"my list" the narrow _CAP_TASK_RE misses); the ACTION
  // requirement stays so a no-action chat turn ("what are my tasks in life") is NOT stolen.
  if ((_CAP_TASK_RE.test(m) || _crud === "tasks") && _CAP_ACTION_RE.test(m)) {
    return {
      money: false,
      lane: "task",
      reply: ar
  ...
  // NOTES — noun + an action signal. Build-159: registry broadens the noun (rescues
  // "fyi …"/"for the record …" the narrow _CAP_NOTE_RE misses); ACTION gate stays.
  if ((_CAP_NOTE_RE.test(m) || _crud === "notes") && _CAP_ACTION_RE.test(m)) {
    return {
      money: false,
      lane: "note",
      reply: ar
...
async function resolveDomainRoute(baseMessage, history) {
  try {
    // (1) Pending clarifier: M8 just asked "wallet or fleet?" and the user replied
    // with a bare pick → route the ORIGINAL question to the chosen domain.
    if (_arbiter.lastWasClarifier(history)) {
      const pick = _arbiter.pickedDomain(baseMessage);
      if (pick) {
        const orig = _arbiter.originalQuestion(history) || baseMessage;
        return { arb: { domain: pick, confidence: 1, why: "clarified" }, routedMessage: orig, clarified: true };
      }
    }
    ...
    // Build-155/156/157: ALL-DOMAIN registry classifier. Computed ONCE when ANY of the
    // dormant shadow (M8_REGISTRY_ROUTER), the lookup flip (M8_REGISTRY_LOOKUP), OR the
    // wallet/fleet/finance CRUD flip (M8_REGISTRY_CRUD) is on.
    //   • M8_REGISTRY_ROUTER=1 → shadow-log only (lane=reg:*), changes NO routing (B-155).
    //   • M8_REGISTRY_LOOKUP=1 → ACT on the read-only lookup lanes (B-156): attach the
    //     decision as `lookup` ONLY when it is a CLEAR (non-ambiguous) winner in
    //     {knowledge, web, memory}. Those are read-only (no money, no writes), so the
    //     caller SOFT-routes them (falls through on no hit). The wallet/fleet/finance/
    //     tasks/notes/driver_profile lanes are untouched (they flip in B-157/158) and the
    //     wallet⇄fleet `arb` returned above is unchanged. `chat` ⇒ no attach = the
    //     default general-answer path (routing to chat is a no-op by definition); an
    //     ambiguous winner (e.g. knowledge⇄docs, knowledge⇄fleet) ⇒ no attach = the
    //     existing behaviour wins, so the flip never steals a docs/fleet/notes turn.
    // Deterministic-only (useLLM:false) ⇒ free during live testing. Fail-safe: any error
    // leaves `lookup` null ⇒ pre-156 behaviour. Both flags OFF ⇒ byte-for-byte pre-155.
    let lookup = null;
    let crud = null; // Build-159 — the LAST CRUD lanes (tasks/notes/driver_profile)
    const _regRouter = process.env.M8_REGISTRY_ROUTER === "1";
    const _regLookup = process.env.M8_REGISTRY_LOOKUP === "1";
    const _regCrud   = process.env.M8_REGISTRY_CRUD === "1"; // Build-157 V2 — wallet/fleet/finance flip (default OFF)
    if (_regRouter || _regLookup || _regCrud) {
      try {
        const reg = await _arbiter.classifyAll(baseMessage, Object.assign({}, _hints, { useLLM: false }));
        if (reg && reg.domain) {
          if (_regRouter) {
            logRoute(baseMessage, "reg:" + reg.domain, (reg.why || "registry") + (reg.ambiguous ? "|amb" : ""), reg.confidence);
          }
          ...
          // Build-157 V2 — FLIP the wallet/fleet/finance CRUD boundary onto the registry.
          // A CLEAR registry winner in {wallet, fleet, finance} REPLACES the B-152 arbiter
          // call so the SAME downstream gate (handleWalletCommand's central guard + the
          // fleet/finance lanes) enforces the registry decision; a wallet⇄fleet money-safety
          // contest ("ask") becomes the clarifier (never a silent guess). Any other registry
          // domain (tasks/notes/knowledge/docs/web/memory/chat) or a non-wallet/fleet tie ⇒
          // leave `arb` (the B-152 wallet⇄fleet call) intact — those lanes own their own
          // front doors and are out of this seam's scope. Default OFF ⇒ byte-for-byte pre-157.
          if (_regCrud) {
            if (reg.domain === "ask") {
            ...
            } else if (!reg.ambiguous && (reg.domain === "tasks" || reg.domain === "notes" || reg.domain === "driver_profile")) {
              // Build-159 — the LAST CRUD lanes onto the registry. UNLIKE wallet/fleet/finance
              // this does NOT replace `arb` (these lanes own their own deterministic front
              // doors and run FIRST). It is carried as `crud` and consumed by the
              // capabilityFallback safety net ONLY after every keyword parser has missed, so
              // it RESCUES a novel/missed phrasing without stealing a turn the keyword lane
              // would have served. Ambiguous/contest ⇒ crud stays null ⇒ pre-159 behaviour.
              crud = { domain: reg.domain, confidence: reg.confidence, why: reg.why || "registry" };
            }
          }
        }
      } catch (_) { /* shadow/lookup/crud must never affect the turn on error */ }
    }

    return { arb, routedMessage: baseMessage, clarified: false, lookup, crud };
  } catch (_) {
    return { arb: { domain: "neutral", confidence: 0, why: "error" }, routedMessage: baseMessage, clarified: false };
  }
}
...
async function orchestrate({ message, sessionId, history, attachments }) {
  ...
  try {
    ...
    const _route = await resolveDomainRoute(baseMessage, history);
    const _arb = _route.arb;
    if (_route.clarified) effectiveMessage = _route.routedMessage; // route the original Q downstream
    if (_arb.domain !== "neutral") { log("arbiter", { domain: _arb.domain, why: _arb.why }); logRoute(baseMessage, _arb.domain, _arb.why, _arb.confidence); }
    ...
    if (intent !== INTENT.DOC) {
      const _cap = capabilityFallback(baseMessage, _arb, _route.crud);
      if (_cap) {
        log("capability_fallback");
        // Build-150/159: log miss for router improvement (fire-and-forget, never blocks reply).
        // The reply now carries its own lane (incl. driver_profile + registry-rescued rows).
        const _missLane = _cap.lane || (_cap.money ? "money" : (_CAP_TASK_RE.test(baseMessage) ? "task" : "note"));
        logMiss(baseMessage, _missLane, _route.crud ? "registry_crud_rescue" : "phase0_safety_net");
    ...
    // resolveDomainRoute attached classifyAll()'s decision as `_route.lookup` ONLY
    // when M8_REGISTRY_LOOKUP=1 AND it is a CLEAR (non-ambiguous) winner in a read-only
    // lookup lane. Default OFF ⇒ `_route.lookup` is null ⇒ this whole block is inert ⇒
    // byte-for-byte pre-156 behaviour. Only "previously-unrouted" turns reach here —
    ...
    if (!financeCtx.text && !eosbCtx.text && _arb.domain !== "wallet") {
      try {
    ...
    if (intent === INTENT.NONE && !imgTurn && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !leanMode && !m1Mode && !m3Mode && !isPersonal(effectiveMessage) && !conversational && !hasDocAttachment && !fleetCtx.text && !financeCtx.text && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !loopCtx.text && !openProblem && !buildQuery && !entityCardSuppressSearch && !forceKnowledgeLookup) {
      try {
    ...
    const kgGateOpen =
      // Build-156: a registry-routed knowledge lane opens the gate even when the LLM
      // tool-router never ran (forced-knowledge suppresses it); fleet/finance/compute/
      // image/ingest exclusions below still apply, so those turns are never stolen.
      ((decision && decision.action === "answer") || forceKnowledgeLookup) &&
    ...
    let topic = null;
    if (intent !== INTENT.NONE && !imgTurn && !computeMode && !compoundMode && !discoveryMode && !oeisMode && !m1Mode && !m3Mode && !fleetCtx.text && !fleetLike && !financeCtx.text && !financeLike && !eosbCtx.text && !companyCtx.text && !stateCtx.text && !notebookCtx.text && !graphCtx.text && !loopCtx.text && !entityCardSuppressSearch && !forceKnowledgeLookup) {
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    // to the (fixed) buffered path. Inert unless M8_REGISTRY_LOOKUP=1 (lookup is null).
    ...
    if (intent !== INTENT.DOC) {
      const _capS = capabilityFallback(baseMessage, _arbS, _routeS.crud);
