 ...
 */
async function extractConceptsWithStatus(source_id) {
  const { data: src, error } = await getDb()
    .from("m8_knowledge_sources")
    .select("raw_text, source_class, title")
    .eq("id", source_id)
  ...
  return { candidates: deduped, quota_exhausted };
}
...
async function ingestBookText({ title, author = null, year = null, text, cls, notes = null, maxChapters = BOOK_MAX_CHAPTERS }) {
  ...
  for (const i of todo) {
    ...
    try {
      const { candidates, quota_exhausted } = await extractConceptsWithStatus(source_id);
