 ...
 */
async function ingestBookText({ title, author = null, year = null, text, cls, notes = null, maxChapters = BOOK_MAX_CHAPTERS }) {
  const db = getDb();
...
function parseBookIngestMessage(message) {
  ...
  return { title, author, year, source_class };
}
...
// ─── Knowledge Graph Search (Build-82) ───────────────────────────────────────
/**
 * Keyword search over m8_graph_nodes. Splits the query into words, searches
 * content and label with ilike, deduplicates, and returns the top `limit` hits
 * as a formatted string for injection into the system prompt.
 * Returns null (never throws) so callers can fire-and-forget safely.
 *
...
async function searchKnowledgeGraph(query, limit = 6) {
  ...
  try {
    const db = getDb();
    // Extract meaningful words (4+ chars, skip Arabic stop-words + English noise)
    ...
    // Build OR filter: each word checked against content and label
    ...
    const { data, error } = await db
      ...
      .limit(limit * 2); // fetch extra, deduplicate below

    if (error || !data || data.length === 0) return null;

    // Deduplicate by label, take top `limit`
    const seen = new Set();
    const hits = [];
