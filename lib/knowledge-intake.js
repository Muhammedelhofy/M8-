...
const KNOWLEDGE_INGEST_DIRECTIVE = `KNOWLEDGE INGEST — present the CLARIFICATION SUMMARY below exactly as written.
Tell Muhammad which nodes were immediately added (high-confidence) and which are pending his review.
Attribute any claim you mention to its source document. Do NOT editorialize or add commentary about the nodes.
Pending nodes can be reviewed via "show me pending extractions" on a future turn.`;
...
async function buildKnowledgeIngestContext(message) {
  ...
  const packet = [
    `KNOWLEDGE INGEST RESULT — source_id: ${source_id}, title: "${title}", class: ${source_class}`,
    `High-confidence nodes written to graph: ${added} added, ${skipped} already existed.`,
    `Pending (medium/low): ${pending_count} node(s) saved for review.`,
    "",
    summary,
    "",
    KNOWLEDGE_INGEST_DIRECTIVE,
