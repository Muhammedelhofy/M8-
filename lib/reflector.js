...
// Saves one row to m8_reflections. Build-110 (Brain CPR): routed through
// safePersist → Vercel waitUntil keeps the lambda alive until the insert flushes
// (the un-awaited fire-and-forget version was silently dropped on the freeze, so
// m8_reflections sat at 0 rows). Still NEVER throws out of this function.
function logReflection(sessionId, question, score, rewritten) {
  try {
    ...
    // Build-110: waitUntil-backed write — response still returns immediately.
    safePersist(db.from("m8_reflections").insert(row), "reflect");
  } catch (e) {
