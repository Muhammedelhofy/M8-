...
// Build-110 (live-verify root cause): gemini-2.5-flash is a THINKING model and its
// hidden thinking tokens ate the 150-token scoring budget — the response truncated
// to ~16 chars ("The answer fails") BEFORE the JSON, so parseScore returned null and
// the reflector NEVER wrote a row. llm.js only disables thinking when thinkingBudget>0
// is sent, and that field 400s on 2.0/1.5-flash, so it can't turn 2.5's default
// dynamic-thinking off. Fix: use a NON-thinking free model so the whole budget is
// available for the actual JSON. Same free Gemini stack. Env-overridable.
const REFLECT_MODEL = process.env.M8_REFLECT_MODEL || "gemini-2.0-flash";
...
async function scoreAnswer(question, answer, sourcesUsed, _sid) {
  ...
  // Build-110 P2 DIAG (temp): record the RAW scoring response so we can tell empty
  // (rate-limit/provider fail) from malformed (parse bug).
  try { await require("./brain-debug").debugBrain("reflector", _sid, "score_raw", "type=" + typeof raw + " len=" + (raw == null ? "null" : String(raw).length) + " :: " + String(raw == null ? "" : raw).slice(0, 180)); } catch (_) {}
...
async function reflect(question, answer, sourcesUsed, opts = {}) {
  ...
  const original = (answer == null ? "" : String(answer));
  await debugBrain("reflector", opts.sessionId, "enter", "len=" + original.length);
  ...
  if (original.length < 200) { await debugBrain("reflector", opts.sessionId, "skip_short", "len=" + original.length); return result; }

  let score = null;
  try {
    score = await scoreAnswer(question, original, sourcesUsed, opts.sessionId);
  } catch (e) {
    await debugBrain("reflector", opts.sessionId, "score_threw", e && e.message); // DIAG
    return result;                                // timeout / provider fail → original
  }
  if (!score) { await debugBrain("reflector", opts.sessionId, "score_null", ""); return result; } // DIAG
  await debugBrain("reflector", opts.sessionId, "score_ok", JSON.stringify(score).slice(0, 200)); // DIAG
  ...
  try {
    await logReflection(opts.sessionId, question, score, result.rewritten);
    await debugBrain("reflector", opts.sessionId, "wrote", "rewritten=" + result.rewritten); // DIAG
  } catch (e) { await debugBrain("reflector", opts.sessionId, "write_threw", e && e.message); }
