...
// getLastM8Write). PRIVACY WALL: selects amount/currency/category/occurred_on
// ONLY — never the `note` free-text — so nothing here can leak counterparty text.
// Read-only (the m8_wallet role's SELECT grant covers it), RLS + household-scoped.
// Returns an array of { amount, currency, category, occurredOn }, newest first.
async function getRecentExpenses(limit = 1, memberId = null) {
...
// Expenses on ONE specific day (privacy-safe: amount/currency/category, NO note),
// optionally scoped to a member. Used by the date-query lane ("...on June 23").
// Returns { amount, currency, category }[] for that occurred_on, newest first.
async function getExpensesByDate(dateISO, memberId = null) {
  const params = {
...
module.exports = {
  getSummary, addExpense, getCategorySpend, getRecentExpenses, getExpensesByDate, getTxnsByRange, getCategoryBreakdown, inferCategory,
