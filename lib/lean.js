 ...
 *   - Formalization is PINNED to Fable 5; if it's unreachable we say so and log
 *     nothing — we never silently substitute a weaker model on this step.
...
- BANNED anywhere in the output: \`#eval\`, \`#check\`, \`axiom\`, \`unsafe\`, \`macro\`, \`set_option\`, and any \`import\` other than \`import Mathlib\`. (The checker rejects these as injection.)
- PROOF POLICY:
...
function narrateDraftFailed() {
  return `I couldn't reach the formalization model (Fable 5) to draft the Lean statement, and I won't substitute a weaker model on the formalization step — so I'm not guessing. Nothing logged. Try again shortly.`;
}
...
async function runLeanTurn({ leanProbe, meta, log } = {}) {
  ...
  const provider = (process.env.LEAN_FORMALIZE_PROVIDER || "anthropic").toLowerCase();
  const model = process.env.LEAN_FORMALIZE_MODEL
    || (provider === "openrouter" ? "anthropic/claude-fable-5" : "claude-fable-5");
