...
function buildLeafDirective(prose, priorError) {
  ...
  if (priorError) {
    ...
    if (/no goals to be solved/i.test(errText)) {
      hint = "\nDIAGNOSIS: 'no goals to be solved' means a tactic ran AFTER the goal was already closed. Find the branch where this happened and remove the extra tactic(s) that appear after the first goal-closer (exact/rfl/ring/omega/simp/norm_num/decide). Each branch needs exactly ONE closer.";
    }
