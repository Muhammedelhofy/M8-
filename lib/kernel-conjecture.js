...
const TEMPLATES = ["dr_periodic", "dr_constant", "mod_cycle"];
...
function renderKernelConjecture(kernel, claim, result) {
  ...
  return `${head}\n\n❌ FALSIFIED. First counterexample at n = ${ce.n}` +
    ...
    (ce.dr !== undefined ? ` (digital root ${ce.dr} ≠ expected ${ce.expected})` : "") +
    `.\n\nThe kernel-derived claim is FALSE as stated. Recorded as a failed attempt (data, not noise).`;
}
...
const KERNEL_TEST_RE = /\b(?:test|check|verify|extract(?:\s+and\s+(?:test|check))?)\b[^?.!]{0,30}\b(?:kernel|established core|real (?:arithmetic|math|core)|number[- ]pattern|digital[- ]root)\b/i;
function detectKernelTest(message) { return KERNEL_TEST_RE.test(message || ""); }
...
async function runKernelTest(message) {
  try {
    ...
    const dec = await proposeDecomposition("user idea", message);
    if (!dec || !dec.kernel) {
      return "I couldn't isolate a genuinely established arithmetic KERNEL from that idea — so there's nothing checkable to test yet. (I won't invent one; the idea stays speculative.)";
    }
    ...
    return "I hit an error setting up the kernel test and won't guess a result. Try rephrasing the idea, Boss.";
  }
}
...
module.exports = {
  ...
  // llm + narration
  parseClaim, kernelToConjecture, renderKernelConjecture,
  // chat integration
