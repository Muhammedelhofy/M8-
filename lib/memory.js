...
async function recallMemory(currentSessionId, currentMessage = "", limit = 6) {
  ...
  try {
    ...
    const _factCols = "id, role, content, importance, memory_type, trust_level, source_type, created_at";
...
async function saveMemory(sessionId, userMessage, assistantResponse) {
  ...
  try {
    ...
    // Build-137 (C): deterministic relationship capture ("Sara is my wife") — free,
    // reliable, runs alongside the model extractor. Background, never blocks.
    _maybeCaptureRelationship(sessionId, userMessage).catch(() => {});
