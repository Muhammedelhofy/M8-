...
async function saveMemory(sessionId, userMessage, assistantResponse) {
  ...
  try {
    ...
    // safePersist registers the whole extraction (Gemini call + inserts) with
    // waitUntil so the lambda stays alive to flush it — response still returns now.
    ...
    // for the Gemini round-trip — this turn would then only persist, not extract.
