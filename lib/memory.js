...
async function saveMemory(sessionId, userMessage, assistantResponse) {
  ...
  try {
    ...
    // Build-83c: extract named entities (people/books/problems/etc).
    // Build-110 (Brain CPR): the un-awaited fire-and-forget call was frozen out
    // before its m8_entities/m8_entity_mentions inserts landed (both at 0 rows).
    // Live verify proved waitUntil does NOT flush from M8's legacy (req,res)
    // handlers, so we AWAIT the extraction here — but BOUND it (5s) so a slow
    // Gemini extract can't wedge the turn. safePersist still wraps with waitUntil
    // + logs +1 / never throws. This is the one write with real latency cost.
    // TODO (AI Council preferred follow-up, Build B+): move the HEAVY Gemini entity
    // extraction into the nightly cron-summarize so the chat lambda isn't held open
    // for the Gemini round-trip at all — this turn would then only persist.
    try {
      const _entityWrite = safePersist(require("./entity-graph")._maybeExtractEntities(sessionId, userMessage), "entity");
      await Promise.race([_entityWrite, new Promise((r) => setTimeout(r, 5000))]);
    } catch (_) {}
  } catch (err) {
