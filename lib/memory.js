...
async function saveMemory(sessionId, userMessage, assistantResponse) {
  ...
  try {
    ...
    // Build-110 (Brain CPR): AWAIT (bounded) so the inserts land before the lambda
    // freezes — waitUntil doesn't flush from M8's legacy (req,res) handlers. The
    // extractor now runs on GROQ (entity-graph.js), which is fast (~1s), so the
    // per-turn cost is small; the 6s race cap guarantees a slow provider can never
    // wedge the turn. (If this turn-latency ever matters, the AI Council's follow-up
    // is to move the extraction into the nightly cron-summarize instead.)
    try {
      const _ent = safePersist(require("./entity-graph")._maybeExtractEntities(sessionId, userMessage), "entity");
      await Promise.race([_ent, new Promise((r) => setTimeout(r, 6000))]);
    } catch (_) {}
  } catch (err) {
