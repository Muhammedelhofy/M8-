...
async function saveMemory(sessionId, userMessage, assistantResponse) {
  ...
  try {
    ...
    // Build-110 (Brain CPR): best-effort, NON-blocking via safePersist. We do NOT
    // inline-await this: the Gemini extraction is heavy and Gemini is slow in this
    // environment (13-19s observed), so awaiting it would add seconds to EVERY chat
    // turn (the exact TTFB hit the design must avoid). safePersist still tries
    // waitUntil + logs +1 / never throws. NOTE (verified 2026-06-22): this write
    // does NOT reliably land in production — waitUntil doesn't flush from M8's
    // legacy (req,res) handlers. The CORRECT fix (AI Council follow-up) is to move
    // entity extraction into the nightly cron-summarize, whose handler IS fully
    // awaited, so the inserts land with zero chat-turn latency. See Build-110 report.
    try { safePersist(require("./entity-graph")._maybeExtractEntities(sessionId, userMessage), "entity"); } catch (_) {}
  } catch (err) {
