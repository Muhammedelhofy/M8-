...
async function fetchQueue({ states, template, limit } = {}) {
  try {
    ...
    if (error) { console.error("[M8] review-queue fetch error (non-fatal):", error.message); return []; }
    return data || [];
  } catch (err) {
...
async function buildReviewQueueContext(message, sessionId) {
  ...
  try {
    ...
    const rows = await fetchQueue({ states: ["new", "kept"] });   // actionable worklist; dismissed hidden
...
module.exports = {
  ...
  clusterAndRank, renderQueuePacket, renderQueueEmptyPacket, renderTriagePacket, fetchQueueByIds,
