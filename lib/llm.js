...
async function generateGeminiWith(apiKey, model, { systemInstruction, contents, genConfig }) {
  ...
  // ── TEMP VISIONDBG (remove after vision fix) ── log the exact parts shape the
  // SDK receives so we can see if an inlineData part actually reaches the call.
  try {
    const last = Array.isArray(contents) ? contents[contents.length - 1] : null;
    const partShape = (last?.parts || []).map((p) =>
      p?.inlineData
        ? `inlineData(${p.inlineData.mimeType},len=${(p.inlineData.data || "").length})`
        : (typeof p?.text === "string" ? `text(len=${p.text.length})` : `?${Object.keys(p || {}).join("|")}`)
    );
    console.error("[VISIONDBG] geminiSend", JSON.stringify({ model, lastRole: last?.role, partShape }));
  } catch (_) {}
