...
async function generateGeminiWith(apiKey, model, { systemInstruction, contents, genConfig }) {
  ...
  // the user. This is what keeps the code-exec planning preamble out of compute
  // replies now that compute routes through this manual path.
  const parts     = candidate?.content?.parts ?? [];
  const textParts = parts.filter((p) => typeof p.text === "string" && p.text.length > 0 && !p.thought);
