 ...
 *   GROQ_MODEL           default "llama-3.3-70b-versatile"  (decommissioned 2026-08-16 — migrate to a TESTED compatible model before then; gpt-oss-120b returns no text here)
...
async function generateGroq(args) {
  return generateOpenAICompatible({
    ...
    // NOTE: llama-3.3-70b-versatile is decommissioned 2026-08-16. The recommended
    // replacement openai/gpt-oss-120b returned "groq returned no text" via this
    // OpenAI-compat parser (reasoning-format output) — so keep llama-3.3 (works
    // until Aug 16) until a COMPATIBLE model is tested live (qwen, or adapt the parser).
