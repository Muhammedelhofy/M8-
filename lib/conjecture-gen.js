...
/**
 * Build-83d: async wrapper around runConjectureGen that injects verified + dead-end leaf
 * context from the knowledge graph into the returned packet. The pure engine
 * (runConjectureGen) is deterministic and CPU-only; this layer adds Supabase feedback.
 *
 * The feedback context is appended as a FEEDBACK CONTEXT block to the packet so the
 * LLM narrating the results can reference already-proven ground and dead-end patterns
 * without re-deriving what the engine has already learned.
 *
 * Fail-safe: missing credentials, GRAPH_DISABLED=1, or any Supabase error degrades
 * silently to a plain runConjectureGen result with empty feedback fields.
 */
async function runConjectureGenWithFeedback(opts) {
  ...
  // Build-92 + Build-99 + Build-112: closed learning loop, BOTH directions, now GATED.
  // Prepend the VERIFIED CONJECTURE PATTERNS block (what already verified -> propose
  // structurally DIFFERENT) and, below it, the Build-99 AVOID block (structural
  // approaches Lean left `sorry`, so the proposer steers clear and prefers verified
  // approaches). With Build-113 the final packet reads
  // VERIFIED / COMPUTATIONAL EVIDENCE / AVOID / <packet>. Additive + fail-safe: empty patterns
  // / any error leaves the packet exactly as runConjectureGen built it.
  //
  // Build-112 (learn->generate keystone): the PREFER / VERIFIED block is built from the
  // N-verifs-GATED set (getEarnedSuccessPatterns — Grok's gate), so a single flaky Lean
  // verify cannot steer generation; the block stays silent until a technique has verified
  // >= M8_LEARN_MIN_VERIFS times. successPatterns (recent verified, UNgated) is still read
  // and returned for telemetry / back-compat, but no longer drives the prompt.
  //
  // Build-113 (FREE survivor learning — the no-Lean lane): with the Lean checker parked,
  // the earnedPatterns PREFER above stays starved. In parallel, surface the FREE in-process
  // signal — structural templates whose machine-generated conjectures SURVIVED falsification
  // to high bounds — as a SEPARATE, clearly-labeled COMPUTATIONAL-EVIDENCE block. ⚠️ survived
  // != proven: it is prepended BELOW the Lean-proven VERIFIED block and ABOVE AVOID, with its
  // own evidence-not-proof framing, so survivor signal is never laundered into the proven set.
  ...
  try {
    ...
    survivorTemplates = await cm.getSurvivorTemplateStats(db, cm.COLLATZ_PROBLEM_ID);   // Build-113: FREE survivor-EVIDENCE PREFER set
    // Prepend order is bottom-up, so the final packet reads:
    //   VERIFIED (Lean-proven) / COMPUTATIONAL EVIDENCE (survivors) / AVOID / <packet>
    const avoidBlock = failedPatterns.length ? cm.buildAvoidBlock(failedPatterns) : "";
    if (avoidBlock) result.packet = avoidBlock + "\n\n" + result.packet;
    const survBlock = cm.buildSurvivorBlock(survivorTemplates);   // Build-113: evidence, NOT proof; silent until a template earns the productivity gate
