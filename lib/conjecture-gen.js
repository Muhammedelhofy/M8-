...
function runConjectureGen({ testBound, seed } = {}) {
  ...
  // M2 novelty gate v1 — deterministic pass (canonical form / template+slots vs
  // the curated literature seed pack). A hit means the FORM is known math; the
  // finite-bound figure is still machine-derived. Fail-safe: no pack, no labels.
  ...
  const packet = [
    `M3-LITE CONJECTURE GENERATOR v${GEN_VERSION} — RUN RESULTS, COMPUTED DETERMINISTICALLY IN CODE by M8 (not by you). Seed ${usedSeed}; constants mined from the n <= ${fmtNum(trainN)} census; every candidate falsified EXHAUSTIVELY over 2 <= n <= ${fmtNum(N)}.${ft.overflowed ? " (Feature pass aborted early on overflow guard — bounds reflect the honest range.)" : ""}`,
    ``,
  ...
  return {
    ...
    testN: N, trainN, seed: usedSeed, overflowed: ft.overflowed, genVersion: GEN_VERSION,
  };
}
...
module.exports = {
  ...
  generateMinedCohort, generateMatchedBaseline, rankSurvivors, clampTestBound,
  ...
  SEED_DEFAULT, M3_THREAD, GEN_VERSION,
};
