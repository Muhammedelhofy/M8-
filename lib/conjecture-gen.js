...
// A2 leak caught on the FIRST live run (Build-14 hotfix): a class pinned to
// n ≡ 1 (mod 4) has σ(n) = 3 PROVABLY (3n+1 ≡ 0 mod 4 → three steps drop
// below n), so "σ ≤ 4 on n ≡ 1 (mod 12)" survived as a fake conjecture.
...
function makeCandidate(template, rnd, ft, trainN, testN, mined) {
  ...
  switch (template) {
    ...
    case "A_res_total_max": {
      t.m = pick(rnd, MODS);
      // σ is locally pinned on classes ≡ 1 (mod 4) — exclude them for the σ
      // template (live-run A2 catch); σ∞ is non-local, plain odd-class rule.
...
module.exports = {
  ...
  generateCohort, rankSurvivors, clampTestBound, classHasOdd, sigmaClassNontrivial,
