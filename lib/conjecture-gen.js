...
const GEN_VERSION   = 3;        // Build-113: generation is now FEEDBACK-CONDITIONED. A
                                // feedback snapshot (earned-verified + sorry outcomes) can
                                // DOWN-WEIGHT explored template regions so the cohort favors
                                // unexplored ones. Falsification + the Wilson/Newcombe GATE
                                // are UNCHANGED — but output now depends on (seed, snapshot),
                                // so the stamp MUST move (A9 "never compare silently"). An
                                // EMPTY snapshot => mined cohort byte-identical to gen v2 for
                                // the seed; only the stamp + m3_feedback metadata differ.
...
function runConjectureGen({ testBound, seed, feedbackProfile } = {}) {
  ...
  const packet = [
    ...
    ``,
    `FEEDBACK STEERING (Build-113, generation v${GEN_VERSION} — outcome-aware): ${feedback.applied ? `down-weighted ${feedback.downWeighted.length} template region(s) [${feedback.downWeighted.join(", ")}] that already carry an earned-verified or sorry outcome, so this cohort favored UNEXPLORED regions (reproducible per seed + snapshot). This biases GENERATION ONLY — not survival, the gate, or truth; a down-weighted region is "explored elsewhere", NEVER solved/proven/true.` : `no recorded outcome earned steering weight (or none classified to a template) — this cohort is byte-identical to gen v2 for the seed; the Record→Generate wire is closed but SILENT until a technique verifies repeatedly.`}`,
    ``,
...
async function runConjectureGenWithFeedback(opts) {
  ...
  if (process.env.GRAPH_DISABLED === "1" ||
    ...
    return Object.assign(result, { feedbackContext: "", verifiedLeaves: [], deadEndLeaves: [], successPatterns: [], failedPatterns: [], earnedPatterns: [] });
  }
  ...
  let successPatterns = [], failedPatterns = [], earnedPatterns = [];
  let feedbackProfile = { downWeight: [], fromVerified: [], fromFailed: [] };
  try {
    ...
    earnedPatterns  = await cm.getEarnedSuccessPatterns(db, cm.COLLATZ_PROBLEM_ID);   // Build-112: N-verifs-gated PREFER set
    ...
    // Kill switch: M8_GEN_STEER_DISABLED=1 keeps the narration blocks but reverts generation to
    // the unsteered v2 cohort (empty profile) — a surgical rollback without disabling all feedback.
    feedbackProfile = process.env.M8_GEN_STEER_DISABLED === "1"
  ...
  // ── 4. PREFER / AVOID narration blocks (Build-92/99/112). These now MIRROR what also
  //       steered generation in step 1; PREFER stays on the N-verifs-gated earned set so a
  //       single flaky verify can move neither the narration nor the cohort. Final packet
  //       reads VERIFIED / AVOID / <packet>. Additive + fail-safe. ──
  try {
    const cm = require("./conjecture-memory");
    const avoidBlock = failedPatterns.length ? cm.buildAvoidBlock(failedPatterns) : "";
    if (avoidBlock) result.packet = avoidBlock + "\n\n" + result.packet;
    const block = cm.buildFeedbackBlock(earnedPatterns);   // Build-112: PREFER steers ONLY on earned (gated) patterns
