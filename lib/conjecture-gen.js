...
async function runConjectureGenWithFeedback(opts) {
  ...
  if (process.env.GRAPH_DISABLED === "1" ||
    ...
    return Object.assign(result, { feedbackContext: "", verifiedLeaves: [], deadEndLeaves: [], successPatterns: [] });
  }
  ...
  // Build-92 + Build-99: closed learning loop, BOTH directions. Prepend the VERIFIED
  // CONJECTURE PATTERNS block (what already verified -> propose structurally DIFFERENT)
  // and, below it, the Build-99 AVOID block (structural approaches Lean left `sorry`,
  // so the proposer steers clear and prefers verified approaches). Prepend order is
  // avoid-then-verified so the final packet reads VERIFIED / AVOID / <packet>.
  // Additive + fail-safe: empty patterns / any error leaves the packet exactly as
  // runConjectureGen built it.
  let successPatterns = [], failedPatterns = [];
  try {
    const cm = require("./conjecture-memory");
    const { createClient } = require("@supabase/supabase-js");
    const db = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_KEY);
    successPatterns = await cm.getSuccessPatterns(db, cm.COLLATZ_PROBLEM_ID);
    failedPatterns  = await cm.getFailedApproaches(db, cm.COLLATZ_PROBLEM_ID);
    const avoidBlock = failedPatterns.length ? cm.buildAvoidBlock(failedPatterns) : "";
    if (avoidBlock) result.packet = avoidBlock + "\n\n" + result.packet;
    const block = cm.buildFeedbackBlock(successPatterns);
    if (block) result.packet = block + "\n\n" + result.packet;
  } catch (e) {
    console.error("[M8] runConjectureGenWithFeedback success-pattern error (non-fatal):", e.message);
  }

  return Object.assign(result, { feedbackContext, verifiedLeaves, deadEndLeaves, successPatterns, failedPatterns });
}
