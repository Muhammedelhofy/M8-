...
async function noveltySemanticPass(survivorStatements, sessionId) {
  ...
  try {
    ...
    const checks = await Promise.all(stmts.map(async (stmt, i) => {
      ...
      const top = ext[0];
      return `- survivor ${i + 1} is semantically CLOSE to a known literature result: "${String(top.label || "").slice(0, 120)}" (cosine ${Number(top.similarity).toFixed(2)}) — possible known-form overlap; the deterministic novelty check is authoritative, this is adjacency only.`;
    }));
