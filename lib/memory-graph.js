...
async function ingestNote(note, opts = {}) {
  try {
    ...
    if (opts.withExtraction) {
      await extractFromNote(note, primary, thread);
    }

    return { ok: true, nodeId: primary ? primary.id : null };
  } catch (err) {
...
function renderGraphPacket(det, matches, edges, farNodes) {
  ...
  lines.push(`Answer Boss's question from this packet ONLY. If the packet doesn't contain what he asked for, say what's missing and offer to log it.`);
  return lines.join("\n");
}
