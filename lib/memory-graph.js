...
async function buildCrossBookContext(topic) {
  ...
  try {
    ...
    const external = (raw || []).filter((n) => n.source === "external");

    if (!external.length) {
      return {
        text: [
          `CROSS-BOOK ANALYSIS — no ingested book nodes match "${topic || "this topic"}".`,
          `Either no books have been ingested yet, or none of the ingested books cover this topic.`,
          `Tip: ingest a book first with "ingest this book as [title] by [author]".`,
        ].join("\n"),
        mode: "crossbook",
