  ...
  try {
    ...
    const ins = await supabase
      ...
      .insert([{
        ...
        source:          (fields.source === "extraction" || fields.source === "external") ? fields.source : "code",
...
function renderGraphPacket(det, matches, edges, farNodes) {
  ...
  let anyGenerated = false, anyExternal = false;
  matches.slice(0, 8).forEach((n, i) => {
    ...
    if (isExternal(n)) { anyExternal = true; bits.push("LITERATURE — cited external result from the curated seed pack, NOT our work"); }
    ...
    lines.push(`${i + 1}. [${n.kind}] ${smartTruncate(n.label || n.content || "", 200)} (${bits.join("; ")})`);
  });
  if (anyGenerated) {
  ...
  if (anyExternal) {
    lines.push(`LITERATURE NOTE: node(s) marked LITERATURE are curated, cited external results (seed pack) — established mathematics by OTHERS. Attribute them to their authors, never to our research${anyGenerated ? "; never merge them with our MACHINE-GENERATED conjectures into one list of \"established results\"" : ""}.`);
  }
...
async function noveltySemanticPass(survivorStatements, sessionId) {
  ...
  try {
    ...
    const checks = await Promise.all(stmts.map(async (stmt, i) => {
      ...
      const top = ext[0];
      return `- survivor ${i + 1} is semantically CLOSE to a known literature result: "${smartTruncate(top.label || "", 120)}" (cosine ${Number(top.similarity).toFixed(2)}) — possible known-form overlap; the deterministic novelty check is authoritative, this is adjacency only.`;
    }));
