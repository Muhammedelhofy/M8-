...
const PRIORITY_RE = /\b(?:what(?:'s| is| should)?\s+(?:the\s+)?(?:my\s+|our\s+|next\s+)?(?:priorit|most important|highest priority|work on next)|prioriti[sz]e|command\s+center|what(?:'s| is)\s+next\s+(?:on|for)\b|where should (?:i|we) focus)\b/i;
