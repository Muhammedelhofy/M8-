...
const BUILD_STATE = {
  ...
  pending: [
    "Compute/search co-fire calibration (sharpened via the tool_decision trace): a math query the regex intent classifier tags RESEARCH/LOOKUP runs the search slot AND compute on the same turn, laundering phantom citations onto a self-computed number (the figure stays correct). Candidate fix: a deterministic gate that suppresses the regex search slot when computeMode/routerCompute fires (compute owns the number) — cleaner than a router-prompt tweak alone",
