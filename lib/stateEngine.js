...
function buildStateContext(message, history) {
  try {
    ...
    if (claim && !claim.present) {
      return {
        text:
          `STATE GROUND TRUTH — CLAIM CHECK (the conversation transcript is the ground truth; do not contradict it):\n` +
          `The user is asserting you played/said "${claim.claimed}". Searching everything you ACTUALLY said earlier in THIS conversation, you never stated "${claim.claimed}". ` +
          `Do NOT confirm it and do NOT back-fill a move/value that wasn't played to be agreeable. Correct the record plainly from what was actually said, then continue. ` +
          `(If you genuinely did make a move and just worded it differently, restate your real move — never invent "${claim.claimed}".)`,
        kind: "claim_check",
        data: claim,
      };
    }
    ...
    if (seq && seq.steps.length >= 2) {           // need an initial + ≥1 op to be useful
      return {
        text:
          `STATE GROUND TRUTH — TALLY LEDGER (deterministically computed from this conversation; state this exact total, do not recompute in your head):\n` +
          `Sequence: ${seq.steps.join(" → ")}\nCurrent total = ${seq.total}`,
        kind: "tally",
        data: seq,
      };
    }
