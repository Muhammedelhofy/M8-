...
async function runVerifyPhase({ runDate = todayUTC(), log = () => {} } = {}) {
  ...
  try {
    ...
    if (row) {
      ...
      // proposer should remember next run. Fire-and-forget — NOT awaited, in its
      // own try/catch, so it can never break or slow the verification path.
      ...
      // proposer's AVOID block steers the next run away. Same fire-and-forget,
      // NOT-awaited, non-fatal discipline as the verified path above.
