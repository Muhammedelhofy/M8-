 ...
 *     step 2  M3 test      -> runConjectureGenWithFeedback({seed: SEED_BASE+dayIndex}) — PURE-CODE gen, NO LLM; READS recorded outcomes to steer (Build-112)
...
async function runObservePhase({ runDate = todayUTC(), bound = DEFAULT_BOUND, log = () => {} } = {}) {
  ...
  try {
    ...
    log("l5_m3", { mined: m3.counts.mined, survivors: m3.counts.minedSurvived, gatePass: m3.gate.pass, seed, genVersion: m3.genVersion, earnedPatterns: row.metadata.learn.earned_patterns });
