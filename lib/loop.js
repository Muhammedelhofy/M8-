 ...
 *     step 2  M3 test      -> runConjectureGenWithFeedback({seed: SEED_BASE+dayIndex}) — PURE-CODE gen, NO LLM; READS Lean-proven outcomes (Build-112) + FREE survivor evidence (Build-113) to steer
...
async function runObservePhase({ runDate = todayUTC(), bound = DEFAULT_BOUND, log = () => {} } = {}) {
  ...
  // now routed through runConjectureGenWithFeedback (Build-112, learn->generate keystone)
  // so the run READS recorded outcomes and the N-verifs-gated PREFER + AVOID blocks steer
  // the packet. Still NO LLM in the loop — the feedback is pure Supabase reads; the only
  // LLM (tag extraction) lives on the WRITE side (recordOutcome, verify phase). Fail-safe:
  // runConjectureGenWithFeedback degrades to a plain run on any Supabase error.
  ...
  try {
    ...
    row.metadata.learn = {
      ...
      // Build-113: FREE survivor-EVIDENCE lane (separate from the Lean-PROVEN patterns above).
      // survivor_templates = templates that earned the productivity gate (computational
      // evidence, NOT proof); survivor_template_tags lists them so the wire is auditable.
