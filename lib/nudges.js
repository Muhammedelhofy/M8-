...
function messageFor(bucket, d, brief) {
  ...
  switch (bucket) {
    ...
    case "reEngage":
      return `كابتن ${d.name}، نشاطك هالشهر قليل (${d.daysOnline || 0} أيام). كل يوم على الطريق يقرّبك من هدف الـ ${fmtMoney(T)} ريال والمكافأة. موجودين لو تحتاج أي مساعدة 💪`;
    default:
      return "";
  }
}
...
async function computeNudges() {
  try {
    ...
    return buildNudges(brief, entries);
  } catch (err) {
