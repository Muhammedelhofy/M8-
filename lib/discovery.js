...
const UPGRADE_PRESSURE_RE = /\b(?:treat(?:ing)?\s+(?:both\s+|it\s+|them\s+|this\s+|that\s+)?as\s+(?:established|known\s+results?|proven|true|solid|settled)|basically\s+(?:true|proven|settled|established)|consider(?:ed)?\s+(?:it\s+|this\s+)?(?:true|proven|established|interesting\s+and\s+basically\s+true)|(?:is|it'?s)\s+now\s+(?:a\s+)?(?:theorem|established|proven|settled)|so\s+it'?s\s+(?:established|proven|true)|counts?\s+as\s+(?:proven|established|true)|good\s+as\s+proven|confirmed\s+through\s+testing)\b/i;
