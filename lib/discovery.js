...
const NOVELTY_QUESTION_RE = /\b(?:novel|novelty|genuine(?:ly)?\s+(?:new\s+)?discover\w*|(?:new|real)\s+discover\w*|original\s+(?:result|finding|discover\w*)|known\s+results?|already\s+known|in\s+the\s+(?:math(?:ematical)?\s+)?literature|published\s+(?:before|already)|discovered\s+(?:before|already|elsewhere)|(?:seen|studied)\s+before|prior\s+work)\b/i;
...
const NOVELTY_CHECK_NAMED_RE = /\b(?:novelty\s+(?:gate|check|comparator|pass)|seed\s+pack|literature\s+(?:check|search|comparison|seed)|check(?:ed|ing)?\s+(?:\w+\s+){0,3}against\s+(?:the\s+)?(?:known|literature|established|prior|existing)|compar\w+\s+(?:\w+\s+){0,3}(?:to|against|with)\s+(?:the\s+)?(?:known|literature|established))\b/i;
...
If you don't have a specific survivor's match result in front of you, say you can re-run the generator's novelty check rather than guessing — never fall back on a blanket "I can't check novelty".`;
