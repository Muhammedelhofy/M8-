  ...
  try {
    const raw = await generate({
      systemInstruction:
        ...
        "Write the final answer so the reasoning is VISIBLE: show each step as " +
        "\"Step N: [sub-question] → [answer]\", then a final line starting \"Therefore:\" with the conclusion. " +
        "Keep each step tight; the synthesis is what carries the full answer. Never invent facts beyond the steps.",
      contents: [{
        role: "user",
        parts: [{ text: `QUESTION: ${String(originalQ || "").slice(0, 800)}\n\nSUB-ANSWERS:\n${stepText}\n\nWrite the step-by-step final answer now.` }],
      }],
      providerOrder: CHAIN_ORDER,
...
async function runChain(originalQ, kgContext, entityCtx, sessionId) {
  ...
  try {
    ...
    const finalAnswer = await synthesize(originalQ, steps);
