 ...
 * m8_reasoning_chains. Build-110 (Brain CPR): routed through safePersist so the
 * Vercel waitUntil keeps the lambda alive until the insert flushes — the old
 * un-awaited .then()/.catch() write was dropped on the freeze (table at 0 rows).
 * Never throws; skipped for ephemeral sessions.
 */
function logChain(sessionId, question, steps, finalAnswer) {
  if (isEphemeralSession(sessionId)) return;
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_KEY) return;
  try {
    // Build-110: waitUntil-backed write — runChain still returns immediately.
    safePersist(
      getClient()
