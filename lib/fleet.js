...
function rollup(entries, indices, label) {
...
function renderRollupPacket(r) {
  ...
  return [
    ...
    `Total net: ${fmtMoney(r.net)} SAR. Gross: ${fmtMoney(r.gross)} SAR. Orders: ${fmtMoney(r.orders)}. Online hours: ${r.hours}.`,
...
async function buildFleetContext(message, history) {
  ...
  if (range && range.indices.length) {
    const r = rollup(entries, range.indices, range.label);
