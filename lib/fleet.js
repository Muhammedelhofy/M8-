...
async function buildFleetContext(message, history) {
  ...
  const followup = (!!dateRef || rangeRef(message) || hasVerbCands) && recentlyDiscussedFleet(history);
  ...
  if (entries.length === 0) {
    // No fleet data on record. On a clear fleet turn, give the capability reply
    // (what M8 can read once synced) rather than empty → a fabricated answer.
    return (directFleet || greetingBrief)
