...
const FLEET_PATTERNS = [
  ...
  // "dashboard" in this system unambiguously means the Bolt fleet dashboard.
  // "report" combined with bolt/dashboard/yesterday context → fleet report request.
  /\bdashboard\b/,
  /\b(?:bolt|my|fleet|daily)\s+report\b|\breport\s+(?:for|from|on)\s+(?:bolt|my|the|fleet|dashboard)\b/,
];
...
function looksFleet(message) {
  return isFleetQuery(message) || briefRef(message) || tierWatchRef(message) || cashRef(message) || churnRef(message);
}
...
async function buildFleetContext(message, history) {
  ...
  const directFleet = isFleetQuery(message) || followup || forcedByOverride || forcedByAssertion || tierWatchRef(message) || briefRef(message) || cashRef(message) || churnRef(message);
