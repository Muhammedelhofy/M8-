...
async function getFleetRecord(rowId = SB_ROW_ID) {
  ...
  const rec = await fetchFleetRecord(rowId);
...
const PACE_TARGET_PATTERNS = [
  /\b(?:who|which\s+drivers?)\b[^.?!]{0,30}\b(?:hit|reach|make|earn|achieve)\b[^.?!]{0,25}\b(?:\d{4,6}|target|goal)\b/,
  /\b(?:can|will|able\s+to)\b[^.?!]{0,25}\b(?:hit|reach|make|earn)\b[^.?!]{0,20}\b(?:target|\d{4,6}\s*(?:sar|﷼)?|monthly)\b/,
  /\b(?:monthly|month)\s+(?:target|goal)\b/,
  /\bon\s+(?:track|pace)\b[^.?!]{0,30}\b(?:target|\d{4,6}|monthly)\b/,
  /\b(?:target|goal)\s+of\s+\d{4,6}\b/,
  /\bwho\s+(?:can|will|is\s+going\s+to)\b[^.?!]{0,25}\b(?:hit|make|reach|achieve)\b/,
];
...
const DRIVER_RANKING_PATTERNS = [
  /\b(?:all|every|each)\s+drivers?\b[^.?!]{0,35}\b(?:earn\w*|net|chart|graph|rank\w*|list)\b/,
  /\b(?:chart|graph|visuali[sz]e|show|bar\s+chart)\b[^.?!]{0,30}\b(?:all|every|each)\s*(?:the\s+)?drivers?\b/,
  ...
  /\b(?:graph|chart|visuali[sz]e)\b[^.?!]{0,40}\b(?:earn\w*|net)\b[^.?!]{0,30}\b(?:all|drivers?)\b/,
...
function driverMTDRankings(entries, targetYear, targetMonth) {
  ...
  return { month: `${MONTH_ABBR[targetMonth]} ${targetYear}`, daysElapsed, daysRemaining, daysInMonth, isCurrentMonth, completeDaysInBlob: monthIndices.length, rankings };
}
...
function renderDriverRankingPacket(r) {
  ...
  return [
    ...
    rows,
    `A bar chart of all drivers' MTD net earnings will be rendered below — keep your narration to 2-3 sentences max. Do NOT re-list all drivers row by row in your reply.`,
  ].join("\n");
}
...
async function buildFleetContext(message, history) {
  ...
  let driverCands = driverCandidates(message);
  let hasDriverCands = !!(driverCands && driverCands.length);
  // Bare-name clarification reply: "ali alshahrani" after M8 asked "which Ali?".
  // driverCandidates() requires a verb phrase and misses these bare responses.
  // Safe: guarded by recentlyDiscussedFleet + registry check that follows.
  if (!hasDriverCands && recentlyDiscussedFleet(history)) {
    const trimmed = message.trim();
    const wc = trimmed.split(/\s+/).length;
    if (wc >= 2 && wc <= 4 && /^[A-Za-z؀-ۿ\s'-]+$/.test(trimmed)) {
      driverCands = [trimmed];
      hasDriverCands = true;
    }
  }
  ...
  const directFleet = isFleetQuery(message) || followup || forcedByOverride || forcedByAssertion || tierWatchRef(message) || briefRef(message) || cashRef(message) || churnRef(message) || paceTargetRef(message) || driverRankingRef(message);
  ...
  if (!directFleet && !maybeDriver && !greetingBrief) return { text: "", data: null };
