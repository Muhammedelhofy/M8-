...
function driverChurn(entries, indices) {
  ...
  for (const r of byKey.values()) {
    ...
    if (score >= CHURN_FLAG_SCORE) {
      flagged.push({ name: r.name, score, reasons, activeDays: r.active.length, lastActive: r.active.length ? days[r.active[r.active.length - 1]].period : null });
    }
  }
...
module.exports = {
  buildFleetContext,
  isFleetQuery, hasOverrideAttempt, mentionsFleetMetric, assertsFleetFigure,
  fetchFleetRecord,
  decodeHistory,
  missionControl,
  resolveTarget,
