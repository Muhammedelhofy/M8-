...
// ── Single-driver lookup (stops M8 fabricating a named driver's numbers) ──────
// Extract a driver name the user is asking about ("what about Mansour Alshehri",
// "how did Habib do", "tell me about X"). Returns null for non-name candidates.
function driverCandidate(message) {
  const m = (message || "").match(/\b(?:what|how)\s+about\s+([^?.,!\n]+)|\bhow\s+did\s+([^?.,!\n]+?)\s+do\b|\btell me about\s+([^?.,!\n]+)|\bwhat\s+(?:did|happened\s+(?:to|with))\s+([^?.,!\n]+)/i);
  if (!m) return null;
  const c = (m[1] || m[2] || m[3] || m[4] || "").replace(/\s+/g, " ").trim();
  if (!c || c.length < 2) return null;
  if (/^(the|a|an|my|our|your|this|that|it|them|him|her|us|everyone|things?)\b/i.test(c)) return null;
  return c;
}
...
function renderDriverNotFound(candidate, period) {
  return [
    ...
    `Tell Muhammad you don't have a driver by that name and do NOT invent any earnings/stats. It may be an account-HOLDER / real name rather than the Bolt account name — M8 only has the Bolt account names — so ask which Bolt account it belongs to.`,
  ].join("\n");
}
...
async function buildFleetContext(message, history) {
  ...
  const driverCand = driverCandidate(message);
  const followup = (!!dateRef || rangeRef(message) || !!driverCand) && recentlyDiscussedFleet(history);
  ...
  // SINGLE-DRIVER lookup ("what about Mansour?") → real line, or honest not-found
  // (never let the LLM fabricate a driver's numbers).
  if (driverCand) {
    const d = findDriver(entries[tgt.index], driverCand);
    return d
      ? { text: renderDriverPacket(d, entries[tgt.index].period), data: d, period: entries[tgt.index].period, driver: true }
      : { text: renderDriverNotFound(driverCand, entries[tgt.index].period), data: null, error: "driver_not_found" };
  }
...
module.exports = {
  ...
  resolveRange, rollup, rangeRef, extractDates, driverCandidate, findDriver,
