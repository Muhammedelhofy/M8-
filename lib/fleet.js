...
// missing too many phrasings. This calls a fast free LLM (Groq/Cerebras, ~200ms)
...
const WEAK_FLEET_RE = /\b(sar|earn\w*|target|driver|captain|fleet|bolt|month|net|gross|project\w*|rank\w*|chart|graph|goal|expect\w*)\b/i;

async function llmFleetClassify(message, history) {
  if (!WEAK_FLEET_RE.test(message || "")) return null;
  let generate;
...
{"fleet":true|false,"intent":"pace_target"|"mtd_ranking"|"mission_control"|"driver_series"|"brief"|"tier"|"cash"|"churn"|"none","target_sar":number|null}
...
target_sar: the monthly earnings goal in SAR if the user named one (e.g. 5000 from "5000 SAR target"), else null.`;
  ...
  try {
    const out = await generate({
      ...
      providerOrder: process.env.ROUTER_PROVIDER_ORDER || "groq,cerebras,gemini",
