...
function findDriver(entry, candidate) {
  ...
  // is on the roster → null (honest not-found), never the wrong driver's numbers.
...
function buildMorningBrief(entries) {
  const todayKey = ymdKey(riyadhTodayYMD());
  ...
  return { mc, week, tw, period: mc.period };
}
...
async function buildFleetContext(message, history) {
  ...
  const directFleet = isFleetQuery(message) || followup || forcedByOverride || tierWatchRef(message) || briefRef(message);
  ...
  if (briefRef(message)) {
    ...
    if (brief) return { text: renderBriefPacket(brief), data: brief, period: brief.period, brief: true };
  }
