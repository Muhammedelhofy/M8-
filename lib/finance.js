...
const FLEET_FINANCE_CONFIG = {
  modelAwarePnL: false, // set to true to activate model-aware revenue (Build P2)
};
...
function computeDriverPnLV2(name, monthKey, entries, profiles, overrides) {
  ...
  return {
    ...
    income, driverNet,
    acctRent, carRent, salary, fleetCut, other, netPnL,
...
function renderFleetPnLPacket(r, bonusPacket) {
  ...
  if (r.modelAware) {
    ...
    lines.push(
      `MODEL-AWARE MODE (Build P2): R-model drivers (${rCount}) show rent revenue only — their Bolt net is their own money, not company revenue. F/S-model drivers unchanged.`
    );
  }
  lines.push(
