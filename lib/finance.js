...
function renderFleetPnLPacket(r) {
  const t = r.totals;
  const lines = [
  ...
  if (withProfile.length) {
    ...
    lines.push(`FINANCIAL ANALYSIS INSTRUCTION: Do NOT just recite the numbers above. Analyze them — identify what's working, what's a risk, and what Muhammad should consider. Lead with the fleet margin, then call out the outliers (best deal, worst deal, any negative-margin drivers). Close with 1-2 concrete recommendations: e.g. which deal structure is most profitable, whether a negative-margin driver's deal needs review, or what the margin spread means for the fleet's financial health. Think like a CFO, not a calculator.`);
  }
...
async function buildFinanceContext(message, history) {
  ...
  try {
    ...
    return { text: renderFleetPnLPacket(r), data: r, mode: "fleet" };
  } catch (err) {
