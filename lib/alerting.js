/**
 * M8 Stateful Alerting — lib/alerting.js  (Build-20 + Build-21)
 *
 ...
 * buildAlertText / detectAlertAck are IO-free and PS-mirror-testable.
 * IO lives in evaluateAlerts / applyAcks.
 */

const { getFleetRecord, decodeHistory, tierName, COACH_ACCEPT, COACH_FINISH } = require("./fleet");
 ...
 * in spec §3 priority order: cash_gap > tier_slip > tier_watch.
 * Pure — no IO.
 *
...
function buildAlertText(openAlerts) {
  ...
  const byCondition = { cash_gap: [], tier_slip: [], tier_watch: [] };
...
async function fetchOpenAlerts() {
  const rows = await sbFetch(`fleet_alerts?condition=in.(cash_gap,tier_slip,tier_watch)&state=not.eq.resolved&select=*`);
...
/**
 * Evaluate cash-gap, tier-slip, and tier-watch alerts for all drivers in the
 * current fleet data. Piggybacks the fleet record cache — free if called after
 * buildFleetContext. Skips eval sessions and fails SAFE (returns [] on any error).
 *
