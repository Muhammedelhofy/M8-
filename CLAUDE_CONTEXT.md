...
- `lib/memory-graph.js` — **Build-10 (2026-06-12, Fable-5 sprint Session 1): Research Memory Graph.** Typed nodes (conjecture/theorem/evidence/counterexample/failed_attempt/technique/sequence/research_thread) + typed edges (supports/contradicts/generalizes/depends_on/formalizes/derived_from) + pgvector embeddings (gemini-embedding-001 @768). Deterministic code-owned ingest at `persistNote()`; Fable-authored, Gemini-executed extraction in the nightly cron sweep; `m8_graph_match` RPC + retrieval core shipped (chat wiring = Session 2). Migration `migrations/memory_graph.sql` APPLIED live. Design + adversarial review: `BUILD_10_SPEC.md`.
