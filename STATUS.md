...
- □ **NEXT** — (a) router calibration: it sometimes prefers SEARCH over compute for arithmetic dressed in a real-world concept (a "bill-split" query searched calculator sites + cited [1,2]; numbers stayed correct) — tighten the compute-vs-search boundary in the router prompt; (b) fresh full 30+2-probe baseline (NOT back-to-back); (c) lift the contract onto future tools (calendar/Lean) as they join.
