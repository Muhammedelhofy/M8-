...
*The single source of truth for "where are we." Updated every session. If this disagrees with a chat or memory, THIS wins. Last updated: 2026-06-10 (North-Star correction + L4 Build-4 Tool Decision Layer + Build-5 probes + Build-6 compute/search gate [team-unanimous] — all live-verified).*
...
L1 ✅ · L2 ✅ (solid) · **L3 🟢 ~85%** · **L4 🟡 ~55%** (compute + auto-route + output contract + tool-decision layer + truth-ownership gate all live; traced) · L5 ⚪ ~5%
...
- ★ **NEXT BUILD — Research Notebook (persistent research memory)** (decided 2026-06-10, team + Claude). The highest-leverage, most-tractable next piece and the substrate L5 needs. Extend the existing Supabase memory spine into a STRUCTURED research ledger: `conjecture · evidence · counterexample · dead-end · status · next-step` (one record per line of inquiry, supersession-aware like the fact store). Without it every session restarts from zero — it's what turns clever single turns into actual research, and it's the one capability the big labs can't give Muhammad (they have no memory of HIS exploration). Deterministic + honest (a ledger of verified facts and dead-ends, not a hallucination surface).
- □ **Then** — (Phase 3) a MINIMAL Lean verification probe (M8 proposes → a checker accepts/rejects; formalize KNOWN theorems first, NOT "prove Riemann") · (Phase 4) computational-discovery loops (Collatz/Goldbach/OEIS to huge N — they feed the notebook) · (Phase 5) long-horizon exploration. Fast-follows: chained search→compute tool; fresh full 33-probe baseline (NOT back-to-back); calendar tool.
