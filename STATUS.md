...
- □ **NEXT** — (a) **compute/search co-fire (sharpened via the tool_decision trace):** a math query the regex intent classifier tags as RESEARCH/LOOKUP runs the search slot AND compute on the same turn (e.g. "9 to the power of 11?" traced `tool_decision=compute` BUT `search_fired=true` → reply added "confirmed by MathCelebrity"; "split the 87,346 bill" searched calculator sites). The computed number stays correct, but search results launder phantom citations onto a self-computed answer. Candidate fix is a DETERMINISTIC gate — when `computeMode`/`routerCompute` fires, suppress the regex search slot (compute owns the number) — cleaner than a router-prompt tweak alone. (b) fresh full 30+2-probe baseline (NOT back-to-back); (c) lift the contract onto future tools (calendar/Lean) as they join.
