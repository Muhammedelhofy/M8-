...
- □ **NEXT** — (a) chained search→compute as a real tool (feed a searched live value into the sandbox in one turn — sequential ownership, the capability behind Gemini's compound question); (b) fresh full 33-probe baseline (NOT back-to-back); (c) lift the contract onto future tools (calendar/Lean) as they join.
